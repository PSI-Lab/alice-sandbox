...
def process_row(seq, one_idx, n_sample, model):
    ...
    target[one_idx] = 1
    pred = model.predict_one_step_ar(seq=seq, n_sample=n_sample, start_offset=2)  # hard-coded offset for now
    ...
    return data_pred, data_sensitivity, data_ppv, data_f
...
def main(model_file, dataset_file, n_sample, output):
    ...
    df = add_columns(df, ['pred_idx', 'sensitivity', 'ppv', 'f_measure'], ['seq', 'one_idx'],
