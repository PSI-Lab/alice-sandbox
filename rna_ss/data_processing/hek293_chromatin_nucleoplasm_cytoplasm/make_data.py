...
genome = gk.Genome('gencode.v27')  # do not use v29 until we fix the unmarshal() bug

data = []
with gzip.open('raw_data/GSM3310469_293_ch_vivo.out.txt.gz') as f:
    for line in f:
        line = line.rstrip()
        fields = line.split('\t')
...
write_dataframe(metadata, df, 'data/hek293_icshape_ch_vivo.csv')
