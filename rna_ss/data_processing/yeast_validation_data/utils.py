...
class ModSeqBedGraph(object):
    # their output format is a bit weird
    # need to write a new parser
    # I think their files are 1-based TODO check!
    def __init__(self, chrom_sizes, data_plus, data_minus, missing_val=np.nan):
        self.data = self._init_array(chrom_sizes, missing_val)
        self.missing_val = missing_val
        self._add_data(data_plus, strand='+')
    ...
    def _add_data(self, data_file, strand):
        with gzip.open(data_file, 'r') as f:
            for row in csv.DictReader(f, fieldnames=['chrom', 'pos', 'val'], delimiter='\t'):
                chrom = row['chrom']
                pos = int(row['pos'])
                val = float(row['val'])
                self.data[chrom][strand][pos - 1:pos] = val  # assume 1-based
