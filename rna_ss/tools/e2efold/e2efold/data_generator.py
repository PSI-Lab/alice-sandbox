...
class SeqDataGen(object):
    # by Alice
    def __init__(self, seqs, len_max):
        # pad to len_max
        seqs_padded = []
        seq_lens = []
        for s in seqs:
            seq_lens.append(len(s))
            assert len(s) <= len_max
            seqs_padded.append(s + 'N' * (len_max - len(s)))
        # encode
        self.data_x = np.stack([seq_encoding(s) for s in seqs_padded])
        print(self.data_x.shape)
        self.len = len(seqs)
        self.seq_length = seq_lens
        # for convenience
        self.contact_dummy = np.zeros([len_max, len_max])

    def get_one_sample(self, index):
        return self.contact_dummy, self.data_x[index], self.contact_dummy, self.seq_length[index]
