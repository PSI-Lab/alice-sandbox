...
\begin{itemize}[leftmargin=*]
    ...
    \item Two sets of 1D conv layers on the one-hot-encoded sequence are run in parallel,
...
interaction between any two positions in the sequence. (TODO number of layers, filter size)

    \item Multiple 2D feature maps, each produced by pairs of 1D activations at a specific layer,
are concatenated (Fig \ref{fig:nn_arch_1} (b) to (c)),
followed by several 2D conv layers (Fig \ref{fig:nn_arch_1} (c) to (d), showing only one layer for simplicity).
%Note that although not shown explicitly on the plot (to keep it simple), multiple 2D conv layers...stacked...



    \item  Activation of the last 2D conv layer is concatenated with output from the previous "timestamp" $y^{t-1}$,
and passed through multiple layers of masked 2D conv, as shown in Fig \ref{fig:nn_arch_1} from (e) to (f)
(showing only one layer for simplicity),
which masks "future" (top right) timestamps.

    \item Finally, a fully connected layer along the feature dimension
   with sigmoid activation produces an output between 0 and 1,
    for each position in the upper triangular matrix, as illustrated in Fig \ref{fig:nn_arch_1} (f) to (e).
...
we initialize the output structure with a matrix of all zeros, concatenate it with the 2D feature map,
...
Sequences are generated uniformly randomly using A,C,G,U (TODO add this to training set dexscription)
and lengths are between $10$ and $100$.
For each sequence in the test set, we ran RNAfold (TODO more details) and sampled $100$ structures from the ensemble.
Then we used our model to also sample $100$ from the output distribution conditioned on the input sequence.
To avoid evaluating on low probability structures, we discarded structures that only occur once.
For each unique structure produced by RNAfold, we computed sensitivity (TODO definition) and
...
as shown in Fig \ref{gradient_ascent}(a).
