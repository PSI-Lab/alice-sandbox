...
State-of-the-art methods based on dynamic programming.
...
Specifically, given a input sequence with arbitrary length $L$: $\bm{x} = x_1, x_2, \dots x_{L}$,
we want to predict a distribution of structures conditioned on the sequence $P(\bm{Y} \mid \bm{x})$ (TODO use the above notation).

We factorize the generative process as follows:

$$
P(\bm{Y} \mid \bm{x}) = P(\{y_{i, i+1}\}_{i=1, 2, \dots L-1} \mid \bm{x}) P(\{y_{i, i+2}\}_{i=1, 2, \dots L-2} \mid \bm{x}, \{y_{i, i+1}\}_{i=1, 2, \dots L-1}) \dots P(y_{i, j} \mid \bm{x}, \{y_{todo}\})
$$

Intuitively, this correspond to Fig ???.
Binary
More details.
...
TODO plot for upper triangular generation process (just draw different slices,
using the above simple example)
