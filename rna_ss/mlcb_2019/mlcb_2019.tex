...
It it unclear whether the model generalizes to other types of RNAs.
...
Masking ensures that the output node only has access to
    ...
    which enables the model to generate output in autoregressive fashion in inference mode.
...
To compare the two structure distributinos,
...
\begin{wrapfigure}{R}
    ...
    \caption{Secondary structure of mouse mammary tumor virus (MMTV): (a) measured by nuclear magnetic resonance (NMR), plot from \cite{staple2005pseudoknots}, (b) predicted by RNAfold, (c) Structures generated by our model (only showing unique structures).}
    \vspace{-3em}
...
After all $100$ iterations, we convert the one-hot-encoding back to a sequence
