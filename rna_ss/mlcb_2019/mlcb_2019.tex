...
without having any assumptions on the types of local structure and without hard-coded parameters,
...
\begin{itemize}
    ...
    \item Two sets of 1D convolution layers on the one-hot-encoded sequence are run in parallel,
...
via inner product along the feature dimension, as shown in Fig \ref{nn_arch_1} from (a) to (b).
...
interaction between any two positions in the sequence. (TODO number of layers, filter size)
    ...
    \item Multiple 2D feature maps, each being the inner product of 1D activations at a specific layer,
are concatenated (Fig \ref{nn_arch_1} (b) to (c)),
followed by a several 2D conv layers (Fig \ref{nn_arch_1} (c) to (d)).
%Note that although not shown explicitly on the plot (to keep it simple), multiple 2D conv layers...stacked...



    \item  Activation of the last 2D conv layer is concatenated with target from the previous "timestamp" $y^{t-1}$,
and the output is generated by an upper triangular convolution, as shown in Fig \ref{fig:nn_arch_1} from (e) to (f),
which masks "future" timestamps and ensure the output is generated in auto-regressive fashion.
   ...
   with sigmoid activation to produce an output between 0 and 1,
    for each position in the upper triangular matrix, as illustrated in Fig \ref{nn_arch_1} (f) to (e).
