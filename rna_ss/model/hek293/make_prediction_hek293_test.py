"""
Make prediction on HEK293 dataset (WIP)
"""
import os
# import sys
# import imp
...
def main(in_file, out_file, config):

    metadata, df = read_dataframe(in_file)
    ...
    def _get_seq(transcript, genome):
        diseq = DisjointIntervalsSequence(transcript.exons, genome)
        return diseq.dna(diseq.interval)

    df = add_column(df, 'sequence', ['transcript'], lambda x: _get_seq(x, genome))
