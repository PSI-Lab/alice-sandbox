...
def residual_unit(l, w, ar, n_repeat_in_residual_unit, residual=True, gated=True, l1=0.0, l2=0.0):
        ...
        for _ in range(n_repeat_in_residual_unit):
            ...
            if not gated:
                ...
                conv = Conv1D(l, w, dilation_rate=ar, padding='same',
                              kernel_regularizer=regularizers.l1_l2(l1=l1, l2=l2))(act)
            else:
                ...
                act_tanh = Conv1D(l, w, dilation_rate=ar, padding='same', activation='tanh',
                                  kernel_regularizer=regularizers.l1_l2(l1=l1, l2=l2))(bn)
...
def build_model_dense_net(config):
...
# def build_model_res_net(L, residual_conv, n_repeat_in_residual_unit, skip_conn_every_n,
#                 residual=True, skipconn=True, gated=True):
def build_model_res_net(config):
    context = resolve_contex_old(config['residual_conv'], config['n_repeat_in_residual_unit'])

    input0 = Input(shape=(None, 4), name='input0')
    conv = Conv1D(config['initial_num_filter'], 1)(input0)
    skip = Conv1D(config['initial_num_filter'], 1)(conv)  # TODO ?

    for i, layer_config in enumerate(config['residual_conv']):
        conv = residual_unit(layer_config['num_filter'], layer_config['filter_width'], layer_config['dilation'],
                             config['n_repeat_in_residual_unit'], config['residual'], config['gated'],
                             config['penalty_l1'], l2=config['penalty_l1'])(conv)

        if (i + 1) % config['skip_conn_every_n'] == 0 or (i + 1) == len(config['residual_conv']):
            if config['skipconn']:
                # Skip connections to the output after every 4 residual units
                dense = Conv1D(config['initial_num_filter'], 1)(conv)  # TODO ?
                skip = add([skip, dense])
            else:
                skip = Conv1D(config['initial_num_filter'], 1)(conv)
    ...
    for n_units in [50, 10]:
        hid = Conv1D(n_units, 1, activation='relu',
                     kernel_regularizer=regularizers.l1_l2(l1=config['penalty_l1'], l2=config['penalty_l1']))(hid)
    output0 = Conv1D(3, 1, activation='sigmoid')(hid)
