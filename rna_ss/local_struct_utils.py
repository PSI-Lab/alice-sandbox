...
#     target_iloop2_on = np.zeros_like(structure_arr)
...
#     mask_iloop2_on = np.zeros_like(structure_arr)
...
#             if np.all(target_iloop1_on[x0:x0 + wx, y0:y0 + wy] == 1):
#                 # set loop 2
...
#             else:
#                 # make sure loop 1 is not set yet (except for top right corner, which in rare cases belong to
#                 assert np.all(target_iloop1_on[x0:x0 + wx, y0:y0 + wy] == 0)
#                 # set loop 1
