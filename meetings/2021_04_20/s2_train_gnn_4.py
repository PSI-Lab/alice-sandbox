...
class Net(torch.nn.Module):
    def __init__(self, num_hids, k, embed_dim):
        ...
        self.gcn = [GATEConv(in_channels=embed_dim, out_channels=num_hids[0], edge_dim=2)]
        for num_hid_prev, num_hid in zip(num_hids[:-1], num_hids[1:]):
            self.gcn.append(GATEConv(in_channels=num_hid_prev, out_channels=num_hid, edge_dim=2))
