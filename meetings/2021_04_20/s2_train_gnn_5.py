...
def main(input_data, training_proportion, learning_rate, num_hids, epochs, batch_size, log_file, kmer, embed_dim, max_len):
    logger = get_logger(log_file)

    df = pd.read_pickle(input_data)

    # subset to shorter sequences!
    ...
    df_tr = df[:n_tr]
    df_va = df[n_tr:]
    logger.info("n train: {}, n validation: {}".format(len(df_tr), len(df_va)))
...
if __name__ == "__main__":
    ...
    main(args.input_data, args.training_proportion, args.learning_rate, args.hid,
         args.epochs, args.batch_size, args.log, args.kmer, args.embed_dim, args.max_len)
