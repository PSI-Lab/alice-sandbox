...
class EdgeUpdate(torch.nn.Module):
    ...
    # TODO be careful with undirected edge - we don't want to double count
    # TODO be careful with backbone edges - do we want to update their embeddings?

    def __init__(self, node_dim, edge_in_dim, edge_out_dim):
        super(EdgeUpdate, self).__init__()
        self.lin = Linear(2 * node_dim + edge_in_dim, edge_out_dim)
    ...
    def forward(self, node_feature, edge_index, edge_feature):
        ...
        node_feature_outer = torch.cat([node_feature1, node_feature2], axis=2)  # n_node x n_node x (n_node_dim*2)

        # select entries that correspond to edges
        # note that each edge is directed (undirected edge has two entries)
        node_pair_feature_edge = node_feature_outer[(edge_index[0], edge_index[1])]  # n_edge x (n_node_dim*2)

        # TODO should be invariant to the two nodes
