...
import copy
import numpy as np
from .misc import add_column, add_columns
import sys
...
def add_bb_bottom_left(df):
    ...
    df = add_columns(df, ['bl_x', 'bl_y'], ['bb_x', 'bb_y', 'siz_x', 'siz_y'], add_bl)
    return df
...
def generate_structs(df_stem, df_iloop, df_hloop):
    ...
    for gs in valid_global_structs:
        ...
        df_gs = add_columns(df_gs, ['n_proposal', 'prob_median'], ['prob'], lambda x: (len(x), np.median(x)))
        # add normalized count (count/upper_bound)
        df_gs = add_column(df_gs, 'n_proposal_norm', ['n_proposal', 'siz_x', 'siz_y'],
...
def ad_hoc_score(df_pred):
    ...
    df_stem = add_column(df_stem, 'score', ['siz_x', 'prob_median', 'n_proposal_norm'],
                             lambda a, b, c: a*b*c)
