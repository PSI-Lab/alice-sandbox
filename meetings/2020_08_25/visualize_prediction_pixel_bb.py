...
def main(in_file, out_file, thres, opt_row, opt_tgt, opt_bb):
        ...
        for _, row in tqdm(row_ite):
            ...
            if opt_bb:
                ...
                fig = make_plot_bb(row['target_stem_on'], row['pred_stem_on'],
                                   ...
                                   title='bounding box prediction, type: {} '.format(row.subset), thres=thres)
                f.write(fig.to_html(full_html=False, include_plotlyjs='cdn'))
...
if __name__ == "__main__":
    ...
    main(args.in_file, args.out_file, args.threshold, args.row, args.tgt, args.bb)
